...
class WshopInstaller extends WopshopWobject {
    ...
    private function createSchema(){
        ...
        $sql = "CREATE TABLE  IF NOT EXISTS {$wpdb->prefix}wshop_sessions (
    ...
    private function setConfiguration(){
        ...
        $sql = 'INSERT INTO `'.$wpdb->prefix.'wshop_config` (`id`, `count_products_to_page`, `count_products_to_row`, `count_category_to_row`, `image_category_width`, `image_category_height`, `image_product_width`, `image_product_height`, `image_product_full_width`, `image_product_full_height`, `video_product_width`, `video_product_height`, `defaultLanguage`, `mainCurrency`, `decimal_count`, `decimal_symbol`, `thousand_separator`, `currency_format`, `use_rabatt_code`, `enable_wishlist`, `default_status_order`,
                ...
                VALUES (1, 12, 3, 1, 160, 0, 100, 0, 200, 0, 320, 240, "'.wopshopGetWPLanguageTag().'", "1", 2, ".", "", 2, 1, 1, 1,
        ...
        $wpdb->query($sql);
        //TODO security key
        $securitykey = md5(get_option('admin_email').time());
        $wpdb->update($wpdb->prefix . 'wshop_config',
            array(
                'securitykey' => $securitykey
            ), 
            array('id'=>1)
            );
        //$config->securitykey = md5(get_option('admin_email').time());
    ...
    private function addPaymentMethods(){
        ...
        $sql = "INSERT INTO `{$wpdb->prefix}wshop_payment_method` (`payment_id`, `name_en-GB`, `name_de-DE`, `description_en-GB`, `description_de-DE`, `payment_code`, `payment_class`, `scriptname`, `payment_publish`, `payment_ordering`, `payment_params`, `payment_type`, `tax_id`, `price`, `show_descr_in_email`) VALUES
                (1, 'Cash on delivery', 'Nachnahme', '', '', 'bank', 'wopshop_pm_bank', '', 1, 1, '', 1, 1, 4.00, 0),
                (2, 'Advance payment', 'Vorauskasse', '', '', 'PO', 'wopshop_pm_purchase_order', '', 1, 2, '', 1, 1, 0, 1),
                (3, 'Paypal', 'Paypal', '', '', 'paypal', 'wopshop_pm_paypal', 'wopshop_pm_paypal', 1, 3, 'testmode=1\n email_received=test@testing.com\n transaction_end_status=6\n transaction_pending_status=1\n transaction_failed_status=3\n checkdatareturn=0', 2, 1, 0, 0),
                (4, 'Debit', 'Lastschrift', 'Please insert your bankdata.', 'Bitte tragen Sie hier Ihre Bankdaten für den Abbuchungsauftrag ein.', 'debit', 'wopshop_pm_debit', 'wopshop_pm_debit', 1, 4, '', 1, 1, 0, 0),
                (5, 'Sofortueberweisung','Sofortueberweisung', '', '', 'ST','wopshop_pm_sofortueberweisung', 'wopshop_pm_sofortueberweisung', 0, 5, 'user_id=00000\nproject_id=00000\nproject_password=00000\ntransaction_end_status=6\ntransaction_pending_status=1\ntransaction_failed_status=3\n', 2, 1, 0, 0)";
        dbDelta($sql);
    }
    ...
    private function addShippingMethodsData(){
        ...
        dbDelta("INSERT INTO `{$wpdb->prefix}wshop_shipping_ext_calc` (`id`, `name`, `alias`, `description`, `params`, `shipping_method`, `published`, `ordering`) VALUES (1, 'StandartWeight', 'wopshop_sm_standart_weight', 'StandartWeight', '', '', 1, 1)");
    }
        ...
        foreach ($languages as $translationLocale){
            ...
            if (!in_array($locale, $shopLangsTag)){
                ...
                if ($ml->addNewFieldLandInTables($locale, $defaultLanguage)){
                    ...
                    if ($show_message){
                        wopshopAddMessage('WOPshop. '.WOPSHOP_INSTALLED_NEW_LANGUAGES.": ".$langNativeName);
                    }
                }
            }
        }
